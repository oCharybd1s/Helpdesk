<?php
	...
	echo "<script> console.log('HELPDESK : allp') </script>";
?>
<div class="row">
				...
				<tbody>
					...
					?>
			    			<tr>
								...
								<td class="mycenter"> 
									<?php
	                                    if($getHistoryHdList[$i]['Status2']=='Selesai'){
	                                      	echo '<img src="../resources/images/done.png" style="width:18px;height:18px;"> '.$getHistoryHdList[$i]['TanggalSelesai2'];
								...
								<?php if($getHistoryHdList[$i]['rating']>=1){ ?>
									<td><?php for($x=0;$x<$getHistoryHdList[$i]['rating'];$x++){ ?>
										<span id="bintang<?php echo $i.'_'.$x; ?>" class="fa fa-star bintang<?php echo $i.'_'.$x; ?> checkedrating" onmouseover="ubahbintang(<?php echo $i; ?>,<?php echo $x; ?>,this);" style="cursor: pointer;"></span>
									<?php } ?>
										<?php for($y=$x;$y<(($x)+(5-$getHistoryHdList[$i]['rating']));$y++){ ?>
										<span id="bintang<?php echo $i.'_'.$y; ?>" class="fa fa-star bintang<?php echo $i.'_'.$y; ?>" onmouseover="ubahbintang(<?php echo $i; ?>,<?php echo $y; ?>,this);" style="cursor: pointer;"></span>
										<?php } ?>
										<input id="tmprating<?php echo $i; ?>" type="hidden" value="<?php echo $getHistoryHdList[$i]['rating']; ?>">
										<input id="simpanrating<?php echo $i; ?>" type="button" class="btn btn-success" value="Simpan" onclick="simpanrating(<?php echo $i; ?>,'<?php echo $getHistoryHdList[$i]['No']; ?>')"/>
									</td>
								<?php }else{ ?>
